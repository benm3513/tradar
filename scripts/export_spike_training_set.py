...
current Phase 4.2 data-quality gates. The intent is to create a training-ready
subset that is honest about sparse history, missing forward labels, and minimum
...
- If label payload columns exist, rows must also have has_label_row = 1 when the
  column is present.
- Rows are kept in full (all columns preserved) unless an explicit column subset
  is requested later.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Useful during Phase 4.2 while quality gates are expected to reject all rows."
        ),
    )
    parser.add_argument(
        "--skip-summary",
...
def build_gate_mask(frame: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame]:
...
def main() -> int:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        gate_mask, gate_details = build_gate_mask(feature_frame)
        ...
        if export_frame.empty and not args.allow_empty_export:
            raise ExportSpikeTrainingSetError(
                ...
                "--allow-empty-export if this is expected during Phase 4.2"
            )
